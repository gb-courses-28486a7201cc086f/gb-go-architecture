...
func TestListItem(t *testing.T) {
	...
	t.Run("Empty filter", func(t *testing.T) {
		filter := &ItemFilter{}
		filter.Limit = len(testItems)
		result, err := db.ListItems(filter)
		if err != nil {
			t.Error("unexpected error: ", err)
		}
	...
	t.Run("Limit+offset", func(t *testing.T) {
		...
		resultLimitMax, err := db.ListItems(&ItemFilter{
			baseFilter: baseFilter{Limit: limitMax},
		})
		if err != nil {
			t.Error("unexpected error: ", err)
		}
		...
		resultOffset0, err := db.ListItems(&ItemFilter{
			baseFilter: baseFilter{Limit: limitMin, Offset: 0},
		})
		if err != nil {
			t.Error("unexpected error: ", err)
		}
		...
		resultOffset1, err := db.ListItems(&ItemFilter{
			baseFilter: baseFilter{Limit: limitMin, Offset: limitMin},
		})
		if err != nil {
			t.Error("unexpected error: ", err)
		}
