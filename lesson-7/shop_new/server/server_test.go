...
func TestSuccessHTTPHandlers(t *testing.T) {
	...
	filter.Limit = 2
	repoItems, err := repo.ListItems(filter)
	if err != nil {
	...
	t.Run("createItemHandler", func(t *testing.T) {
		...
		resp, body, err := doTestReq(client, http.MethodPost, server.URL+"/items", bytes.NewBuffer(testPayload))
		if err != nil {
	...
	t.Run("getItemHandler", func(t *testing.T) {
		...
		testURL := fmt.Sprintf("%s/items/%d", server.URL, expectedItem.ID)
	...
	t.Run("listItemHandler empty params", func(t *testing.T) {
		resp, body, err := doTestReq(client, http.MethodGet, server.URL+"/items", nil)
		if err != nil {
			t.Fatal("unexpected test server error: ", err.Error())
		}
		...
		if result.Limit == 0 {
			// decode result should not be default ListResponse.
			t.Error("unexpected response, expected non empty payload")
		}
	})

	t.Run("listItemHandler with params", func(t *testing.T) {
		testURL := fmt.Sprintf("%s/items?price_left=0&price_right=100&limit=1&offset=1", server.URL)
		resp, body, err := doTestReq(client, http.MethodGet, testURL, nil)
		if err != nil {
			t.Fatal("unexpected test server error: ", err.Error())
		}
...
func TestUserFailHTTPHandlers(t *testing.T) {
	...
	t.Run("createItemHandler 400", func(t *testing.T) {
		...
		resp, _, err := doTestReq(client, http.MethodPost, server.URL+"/items", bytes.NewBuffer(testPayload))
		if err != nil {
			t.Fatal("unexpected test server error: ", err.Error())
		}
	...
	t.Run("getItemHandler 400", func(t *testing.T) {
		resp, _, err := doTestReq(client, http.MethodGet, server.URL+"/items/badID", nil)
		if err != nil {
			t.Fatal("unexpected test server error: ", err.Error())
		}
	...
	t.Run("getItemHandler 404", func(t *testing.T) {
		resp, _, err := doTestReq(client, http.MethodGet, server.URL+"/items/-1", nil)
		if err != nil {
			t.Fatal("unexpected test server error: ", err.Error())
		}
...
func TestServerFailHTTPHandlers(t *testing.T) {
	...
	t.Run("listItemHandler", func(t *testing.T) {
		resp, _, err := doTestReq(client, http.MethodGet, server.URL+"/items", nil)
		if err != nil {
			t.Fatal("unexpected test server error: ", err.Error())
		}
